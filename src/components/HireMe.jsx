...
const HireMe = ({ lang = 'sv', isDark }) => {
  ...
  // (Behåll handleSubmit och sendRealEmail som de var)
  ...
  // ÄNDRING: Dynamiska klasser för inputs baserat på isDark
  const inputClass = `w-full rounded-lg p-3 outline-none transition-colors text-sm border appearance-none
    ${isDark 
      ...
      : 'bg-white border-gray-300 text-gray-900 placeholder-gray-400 focus:border-neon-purple'}`;
  ...
  if (status === 'sent') {
    return (
      <div className="h-full flex flex-col items-center justify-center text-center p-6 animate-fade-in">
        ...
        <h2 className={`text-3xl font-bold mb-2 transition-colors ${isDark ? 'text-white' : 'text-gray-800'}`}>{t.status.successTitle}</h2>
        <p className={`max-w-md mb-8 leading-relaxed transition-colors ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
          {t.status.successMsg} <strong>{formData.email}</strong> {t.status.soon}
        </p>
        <button onClick={() => { setStatus('idle'); setFormData({ ...formData, name: '', email: '', description: '', captcha: '' }); }} className="px-8 py-3 bg-transparent border border-green-500/30 text-green-500 hover:bg-green-500/10 hover:border-green-500 rounded-full font-medium transition-all">
          {t.buttons.new}
        </button>
      </div>
    );
    ...
    <div className="w-full flex flex-col p-1 pb-10"> 
      ...
      {status === 'idle' && (
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <div className="relative">
                <User className={`absolute left-3 top-3 ${isDark ? 'text-gray-500' : 'text-gray-400'}`} size={16} />
            ...
            <div>
              ...
              <div className="relative">
                <Mail className={`absolute left-3 top-3 ${isDark ? 'text-gray-500' : 'text-gray-400'}`} size={16} />
          ...
          <div>
             ...
             <div className="flex gap-4 flex-wrap">
               {t.options.org.map(type => (
                 <label key={type} className={`flex items-center gap-2 cursor-pointer text-sm transition-colors ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
          ...
          <div className={`border-t my-4 transition-colors ${isDark ? 'border-white/10' : 'border-gray-200'}`}></div>
          ...
          <div className="relative">
            ...
            <div className={`absolute right-3 top-[32px] pointer-events-none text-xs ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>▼</div>
          </div>
          ...
          <div>
             ...
             <div className="relative">
                <ShieldCheck className={`absolute left-3 top-3 ${isDark ? 'text-gray-500' : 'text-gray-400'}`} size={16} />
      ...
      {status === 'analyzing' && (
        <div className="flex flex-col items-center justify-center h-64 text-center space-y-4">
          ...
          <p className={`animate-pulse transition-colors ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>{t.buttons.analyzing}</p>
        </div>
      )}
      ...
      {status === 'approved' && (
        <div className="bg-green-500/10 border border-green-500/30 p-6 rounded-2xl text-center space-y-4 animate-fade-in">
          ...
          <h3 className={`text-xl font-bold transition-colors ${isDark ? 'text-white' : 'text-gray-800'}`}>{t.status.approved}</h3>
          <p className={`text-sm leading-relaxed p-4 rounded-lg border transition-colors ${isDark ? 'bg-[#0a0b1e] border-white/5 text-gray-300' : 'bg-white border-gray-200 text-gray-600'}`}>"{analysis?.feedback}"</p>
          <div className="text-xs uppercase tracking-wider transition-colors text-gray-500">{t.status.time}: <span className="text-green-500 font-bold">{analysis?.estimatedHours}{t.status.hours}</span></div>
          ...
          <button onClick={() => setStatus('idle')} className="text-xs text-gray-500 hover:text-green-600 underline pt-2 transition-colors">{t.buttons.change}</button>
        </div>
      )}
      ...
      {status === 'rejected' && (
        <div className="bg-red-500/10 border border-red-500/30 p-6 rounded-2xl text-center space-y-4 animate-fade-in">
          ...
          <h3 className={`text-xl font-bold transition-colors ${isDark ? 'text-white' : 'text-gray-800'}`}>{t.status.rejected}</h3>
          <p className={`text-sm leading-relaxed p-4 rounded-lg border transition-colors ${isDark ? 'bg-[#0a0b1e] border-white/5 text-gray-300' : 'bg-white border-gray-200 text-gray-600'}`}>"{analysis?.feedback}"</p>
          <button onClick={() => setStatus('idle')} className={`w-full py-3 rounded-xl font-bold transition-all border ${isDark ? 'bg-white/10 text-white border-white/10 hover:bg-white/20' : 'bg-white text-gray-800 border-gray-200 hover:bg-gray-50'}`}>{t.buttons.change}</button>
        </div>
      )}
